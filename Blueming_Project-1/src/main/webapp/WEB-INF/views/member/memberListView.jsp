...
<body>
    ...
    <div class="outer">
        ...
        <table class="table table-bordered table-sm">
            <thead>
                <tr>
                    <th onclick="clickSort('MEMBER_ID')">사원번호</th>
                    <th onclick="clickSort('DEPARTMENT_ID')">부서</th>
                    <th onclick="clickSort('POSITION_ID')">직급</th>
                    <th onclick="clickSort('NAME')">이름</th>
                    <th onclick="clickSort('STATUS')">상태</th>
                    <th>관리</th>
                </tr>
            </thead>
            <tbody>
				    <c:forEach var="member" items="${requestScope.list}">
				        <tr>
				            <td onclick="goDetail('${member.memberId}')">${member.memberId}</td>
				            <td onclick="goDetail('${member.memberId}')">${member.deptName}</td>
				            <td onclick="goDetail('${member.memberId}')">${member.positionName}</td>
				            <td onclick="goDetail('${member.name}')">${member.name}</td>
				            
				            <td onclick="goDetail('${member.memberId}')">
				                <c:choose>
				            ...
				            <td>
				                <button class="btn btn-danger btn-sm" onclick="deleteMember('${member.memberId}')">삭제</button>
				            </td>
				        </tr>
				    </c:forEach>
			</tbody>
    ...
    <c:forEach var="p" begin="${requestScope.pi.startPage}" end="${requestScope.pi.endPage}">
        <c:choose>
            <c:when test="${p eq requestScope.pi.currentPage}">
                <span style="background-color: #007bff; color: white;">${p}</span>
            </c:when>
            <c:otherwise>
    ...
    <c:if test="${requestScope.pi.currentPage < requestScope.pi.maxPage}">
        <a href="javascript:void(0);" onclick="pageMove(${requestScope.pi.maxPage})">끝</a>
    </c:if>
</div>

        <button type="button" class="btn-add-member" onclick="document.getElementById('goInsertForm').submit();">+ 사원 추가</button>
